...
public class Browser implements HTTPConnectionFactoryInterface {
    ...
    public static enum REFERRER_POLICY {
        ...
        public static boolean isSecureProtocol(final Request current, final Request next) {
            return StringUtils.equalsIgnoreCase("https", current.getURL().getProtocol()) && REFERRER_POLICY.isSameProtocol(current, next);
        }

        public static boolean isDowngrade(final Request current, final Request next) {
            return StringUtils.equalsIgnoreCase("https", current.getURL().getProtocol()) && !StringUtils.equalsIgnoreCase("https", next.getURL().getProtocol());
        }
        ...
        public static REFERRER_POLICY get(final Request request) {
            ...
            metaElement: {
                ...
                final REFERRER_POLICY ret = REFERRER_POLICY.get(meta);
                if (ret != null) {
                    return ret;
                }
            }
            policyHeader: {
                ...
                if (policyHeader != null) {
                    ...
                    for (String policyValue : policyValues) {
                        final REFERRER_POLICY policy = REFERRER_POLICY.get(policyValue);
                        if (policy != null) {
                            ret = policy;
                        }
                    }
    ...
    public boolean isHtml() {
        ...
        if (req == null || !req.isRequested()) {
            return false;
        }
        return req.containsHTML("^\\s*(<script[^>]*>.*?</script>)?\\s*(<!doctype\\s*html[^>]*>)?\\s*<html");
    }
    ...
    public String setReferrer(REFERRER_POLICY referrerPolicy, final String requestReferrer, Request nextRequest) throws IOException {
        if (referrerPolicy == null) {
            return this.setReferrer(REFERRER_POLICY.STRICT_ORIGIN_WHEN_CROSS_ORIGIN, requestReferrer, nextRequest);
    ...
    protected REFERRER_POLICY getNextRequestReferrerPolicy() {
        REFERRER_POLICY ret = this.getBrowserReferrerPolicy();
        if (ret != null) {
            return ret;
        }
    ...
    public static enum GenericSupportedBlockTypes implements LabelInterface, BlockedTypeInterface {
        ...
        TENCENT_CLOUD_EDGE_ONE {
            @Override
            public String getLabel() {
                /* e.g. used for: huaban.com */
                return "Tencent Cloud EdgeOne";
            }
        ...
        DNS_BLOCK_AGCOM {
            ...
            public BlockedTypeInterface isBlocked(Browser browser, Request request) {
                ...
                if (con.getResponseCode() == 404) {
                     ...
                     * <h1>AVVISO</h1>
                     * <p>
                     * L&rsquo;accesso al presente sito &egrave; stato disabilitato in esecuzione di un provvedimento adottato
                     * dall&rsquo;Autorit&agrave; per le garanzie nelle comunicazioni, ai sensi del regolamento in materia di tutela del
                     * diritto d&rsquo;autore online approvato con delibera n.
                     * <p>
                     * Per maggiori informazioni visiti il sito <a href="http://www.agcom.it">www.agcom.it</a>
                     * </p>
                     * </body>
                     */
                    if (request.containsHTML("<h1>\\s*AVVISO\\s*</h1>") && request.containsHTML("per le garanzie nelle comunicazioni") && request.containsHTML("www\\.agcom\\.it")) {
                        return this;
                    }
                }
                return null;
