...
public class RuleWrapper<T extends FilterRule> {
    ...
    public RuleWrapper(final T rule2) {
        ...
        if (rule.getCommentFilter().isEnabled()) {
            commentRule = new CompiledRegexFilter(rule.getCommentFilter());
        } else {
            commentRule = null;
        }
    ...
    public CompiledRegexFilter getCommentRule() {
        return commentRule;
    }
    ...
    public boolean checkComment(final CrawledLink link) {
        final CompiledRegexFilter rule = getCommentRule();
        if (rule == null) {
            return true;
        }
        final String comment = link.getComment();
        if (StringUtils.isEmpty(comment)) {
            return false;
        }
        final Pattern pattern = getPatternWithoutDynamicTags(link, rule);
        return rule.matches(pattern, comment);
    }
