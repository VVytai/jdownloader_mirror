...
public abstract class K2SApi extends PluginForHost {
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        if (get(this.getConfigInterface()).isEnableReconnectWorkaround()) {
    ...
    public void handleDownload(final DownloadLink link, final Account account) throws Exception {
        ...
        if (!StringUtils.isEmpty(storedDirecturl)) {
            ...
            if (isFree && get(this.getConfigInterface()).isEnableReconnectWorkaround()) {
                /**
    ...
    public void onCaptchaTimeout(DownloadLink link, Challenge<?> challenge) throws CaptchaException, PluginException, InterruptedException {
        final CaptchaTimeoutBehavior captchaTimeoutBehavior = get(Keep2shareConfig.class).getCaptchaTimeoutBehavior();
    ...
    public String getCustomReferer(final DownloadLink link) {
        ...
        if (!StringUtils.isEmpty(url_referer) || !StringUtils.isEmpty(custom_referer)) {
            ...
            try {
                return new URL(chosenReferer).toExternalForm();
            } catch (MalformedURLException e) {
                /* Do not log this as it will happen whenever a user puts a domain into the custom referer field. */
                try {
                    return new URL("https://" + chosenReferer).toExternalForm();
                } catch (MalformedURLException e2) {
                    this.logger.log(e2);
                    logger.warning("Failed to find any valid referer value");
                }
            }
            return null;
    ...
    private void controlSlot(final int num, final Account account) {
        ...
        synchronized (CTRLLOCK) {
            ...
            final int maxSlotsPossible = useAccountMaps ? Math.max(1, account.getMaxSimultanDownloads()) : get(this.getConfigInterface()).getMaxSimultaneousFreeDownloads();
