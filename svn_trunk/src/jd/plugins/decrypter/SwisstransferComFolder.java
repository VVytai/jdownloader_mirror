...
public class SwisstransferComFolder extends PluginForDecrypt {
    ...
    private ArrayList<DownloadLink> decryptIt_D(final CryptedLink param, ProgressController progress) throws Exception {
        ...
        final String linkUUID = new Regex(addedlink, this.getSupportedLinks()).getMatch(1);
        ...
        final SwisstransferComConfig cfg = get(SwisstransferComConfig.class);
        final jd.plugins.decrypter.SwisstransferComFolder.SwisstransferComConfig.CrawlMode crawlMode = cfg.getCrawlMode().getMode();
        ...
        if (ressourcelist.size() > 1 && (!jd.plugins.decrypter.SwisstransferComFolder.SwisstransferComConfig.CrawlMode.FILES_FOLDERS.equals(crawlMode) || forceZipOnly)) {
            ...
            if (jd.plugins.decrypter.SwisstransferComFolder.SwisstransferComConfig.CrawlMode.ZIP.equals(crawlMode) || forceZipOnly) {
    ...
    private ArrayList<DownloadLink> decryptIt_DL(final CryptedLink param, ProgressController progress) throws Exception {
        final ArrayList<DownloadLink> ret = new ArrayList<DownloadLink>();
        final String contenturl = param.getCryptedUrl();
        final String linkUUID = new Regex(contenturl, PATTERN_DL).getMatch(1);
        if (linkUUID == null) {
